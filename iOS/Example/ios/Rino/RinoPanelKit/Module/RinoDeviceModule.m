...
RCT_EXPORT_METHOD(getGroupDevices:(NSString *)groupId
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        for (RinoDeviceModel *deviceModel in deviceList) {
            NSDictionary *dict = [deviceModel mj_JSONObject];;
            NSMutableDictionary *deviceDict = [NSMutableDictionary dictionaryWithDictionary:dict];
            [deviceDict setValue:deviceModel.deviceId forKey:@"id"];
            [deviceDict setValue:@(deviceModel.ipcCameraNum) forKey:@"ipcCameraNum"];
            [deviceDict setValue:@(deviceModel.isOnline) forKey:@"isOnline"];
