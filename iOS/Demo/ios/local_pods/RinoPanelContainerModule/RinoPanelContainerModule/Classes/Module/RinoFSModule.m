...
#import <RinoPanelKit/NSArray+Map.h>
#import <RinoPanelKit/RinoPanelDownloader.h>
#import <RinoPanelKit/RinoPanelUploader.h>

@interface RinoFSModule()
...
- (dispatch_queue_t)methodQueue {
    return dispatch_queue_create("com.smart.rinoiot", DISPATCH_QUEUE_SERIAL);
}
...
RCT_EXPORT_METHOD(getVideoDuration:(NSString *)filePath
    ...
    NSInteger minute = 0;
    NSInteger second = 0;
    AVURLAsset *urlAsset = [AVURLAsset URLAssetWithURL:[NSURL fileURLWithPath:filePath] options:nil];
    // 获取视频的时长（CMTime）
    CMTime duration = urlAsset.duration;

    // 将时长转换为秒，并计算毫秒
    Float64 durationInSeconds = CMTimeGetSeconds(duration);
    second = (NSInteger)(durationInSeconds * 1000);
    if(resolve){
        resolve(@(second));
    }

}
RCT_EXPORT_METHOD(readFileBytes:(NSString *)filepath
                  jsonOptions:(NSString *)jsonOptions
                  resolver:(RCTPromiseResolveBlock)resolve
                  rejecter:(RCTPromiseRejectBlock)reject) {
    ...
    if (data.length > 0) {
        ...
        NSMutableArray *numberArray = [NSMutableArray arrayWithCapacity:length];
        for (NSUInteger i = 0; i < length; i++) {
            NSNumber *number = [NSNumber numberWithUnsignedChar:bytes[i]];
