...
//        mViewModel.getM3u8SignUrl(1, "设备id", date);
    ...
    private void initCloudReview(String url) {
        ...
        addTextView("开启播放", v -> {
            ...
            mViewModel.setPlay(ijkVideoView);
        });

        addTextView("暂停播放", v -> {
            ToastUtil.showMsg("暂停播放");
            mViewModel.setPaused(ijkVideoView);
        });


        mViewModel.setPlaySource(ijkVideoView, url, null);
        mViewModel.setPlay(ijkVideoView);
    }
    ...
    protected void onDestroy() {
        ...
        for (IjkVideoView ijkVideoView : ijkVideoViewList) {
            mViewModel.cloudReviewRelease(ijkVideoView);
        }
    }
